...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CDeftAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CDeftAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CDeftAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CDeftAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CDeftAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CDeftAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CDeftAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CDeftAddress(dest).ToString(), key);
}
