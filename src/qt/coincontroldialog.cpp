...
{
    GUIUtil::setClipboard(DeftUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = DeftUnits::DEFT;
    ...
    l2->setText(DeftUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(DeftUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(DeftUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(DeftUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CDeftAddress(outputAddress).ToString());
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, DeftUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, DeftUnits::format(nDisplayUnit, nSum));
