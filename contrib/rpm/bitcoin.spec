...
Source100:	https://upload.wikimedia.org/wikipedia/commons/4/46/Deft.svg
...
%description
Deft is a digital cryptographic currency that uses peer-to-peer technology to
...
to run a Deft wallet, this is probably the package you want.
%endif


%package libs
Summary:	Deft shared libraries
...
Encoding=UTF-8
Name=Deft
Comment=Deft P2P Cryptocurrency
Comment[fr]=Deft, monnaie virtuelle cryptographique pair à pair
Comment[tr]=Deft, eşten eşe kriptografik sanal para birimi
Exec=bitcoin-qt %u
Terminal=false
Type=Application
Icon=bitcoin128
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 12668
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 12667
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 12668
	%{_sbindir}/semanage port -d -p tcp 12667
