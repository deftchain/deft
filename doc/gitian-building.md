...
*Setup instructions for a Gitian build of Deft using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Deft
...
- [Building Deft](#building-deft-core)
...
Clone the git repositories for deft and Gitian.
...
in the deft repository under 'Fetch and create inputs' to install sources which require
...
To build Deft (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the deft repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/deft/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/deft-project/deft
...
For example:
```bash
URL=https://github.com/thrasher-/deft.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit deft=${COMMIT} --url deft=${URL} ../deft/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit deft=${COMMIT} --url deft=${URL} ../deft/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit deft=${COMMIT} --url deft=${URL} ../deft/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the deft git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../deft/contrib/gitian-descriptors/*|sort|uniq )
...
DEFTPATH=/some/root/path/deft
SIGPATH=/some/root/path/deft-detached-sigs

./bin/gbuild --url deft=${DEFTPATH},signature=${SIGPATH} ../deft/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/deft-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/deft-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/deft-osx-build.assert
```
...
[deft-project/gitian.sigs.ltc](https://github.com/deft-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
