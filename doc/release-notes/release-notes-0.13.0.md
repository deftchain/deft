Deft version 0.13.0 is now available from:
...
Various code modernizations have been done. The Deft code base has
...
HD wallets are incompatible with older versions of Deft.
...
proposed parameters, a future release of Deft is expected that
...
The two phases are distinct in the Deft-Qt GUI. During the first one,
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use DEFT/kB (MarcoFalke)
...
- #8207 `f7a403b` Add a link to the Deft-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [Deft-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
